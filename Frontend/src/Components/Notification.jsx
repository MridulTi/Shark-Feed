...
export function NotificationsMenu() {
  ...
  function getNotify() {
    axios.get('api/v1/posts/get-notify')
      .then(res => {
  ...
  function MarkasRead(){
    axios.get('api/v1/posts/mark-notify')
    .then(res=>{
      getNotify()
    })
    .catch(err=>{
  ...
  return (
    <Menu className="">
      ...
      <MenuList className="flex flex-col gap-2 my-4">
        <Button onClick={MarkasRead} className="bg-gray-5 text-semantics-1 border border-2 border-semantics-1">Mark as Read</Button>
        <div className="max-h-64 overflow-y-scroll outline-0">

        {Notification.length > 0 && Notification.map((notify) => {
          return (
            <MenuItem className={`flex items-center gap-4 pl-2 pr-8 ${notify.read?"":"bg-gray-4"}`}>
              <Avatar
                variant="circular"
                src={notify.User[0].avatar}
              />
              <div className="flex flex-col gap-1">
                <Typography variant="small" color="gray" className="font-semibold">
                 {notify.User[0].fullName} {notify.content}
                </Typography>
